...
def generate_liveness_partitions(round_arrangements, all_nodes, leaders):
    max_round = max(round_arrangements.keys()) + 1
    liveness = []
    liveness.append(all_nodes)
    for id in leaders:
        view = PartitionView(liveness, id, False, None, None)
        round_arrangements[max_round] = view
        max_round += 1
    return
...
def generate_test_scenarios(config_id, configs):
    ...
    generate_liveness_partitions(round_arrangements, all_nodes, leaders)

    tag_intra_partition_drop(round_arrangements, configs)

    fileoutput(all_nodes, honest_nodes, faulty_nodes, round_arrangements, config_id, configs)
...
def enumerate_partitions(partiton_views, config):
    ...
    for round_no, partition_view in enumerate(partiton_views, 0):
        ...
        if round_no >= (config["enumeration_limit"]["total_round_views"] - config["total_nodes"]):  # Enumeration Limit
            break
...
def fileoutput(all_nodes, honest_nodes, faulty_nodes, round_arrangements, config_id, configs):
    file = open("../config/test_scenarios/test_scenarios{}.json".format(config_id), "w")
    output = {"all_nodes": all_nodes,
              "honest_nodes": honest_nodes,
              "faulty_nodes": faulty_nodes,
              "enable_partitions": True,
              "diem_config" : configs["diem_config"],
              "round_arrangements": round_arrangements
              }
...
def main():
    ...
    if os.path.exists('../config/test_scenarios') and os.path.isdir('../config/test_scenarios'):
        shutil.rmtree('../config/test_scenarios')
    os.makedirs('../config/test_scenarios')
    for config_id, config in enumerate(configs):
        generate_test_scenarios(config_id, config)
